...
function get_content($message, $MadelineProto1=null) {
    global $NOW;
    global $BASE_PATH;
    global $MEDIA_PATH;
        ...
        if (isset($message['media']['_']))
        {
            ...
            try {
                ...
                if ($message['media']['_'] === 'messageMediaDocument') {
                    ...
                    print(time() - $NOW . ' ' . "DOCUMENT $media_id $mime_type $size\n");
                    if ($mime_type == 'video/mp4') {
                        if ($size < MAX_FILE_SIZE) {
                            ...
                            $content .= 'видео размером '. $size . ' байт не загружено - максимальный рзмер: ' . MAX_FILE_SIZE . '<br>';
                        }
                    }
                    else {
                        $content .= 'контент '.$mime_type.' размером '. $size . ' байт не загружен<br>';
                    }
                }
...
$wpdb->update( $task_table_name, ['done'=>1, 'done_at'=>current_time('mysql', 1), 'posts_loaded'=>0], ['id'=>$task->id]);
...
foreach (array_reverse($TOREAD) as $message) {
    $post_id = $message['id'] + 0;
    $grouped_id = isset($message['grouped_id']) ? $message['grouped_id'] + 0 : 0;
    ...
    $post['post_content'] .= get_content($message, $MadelineProto);

    if ($grouped_id>0 and isset($GROUPED[$grouped_id])) {
        print (" ------- HAS GROUPED  -----\n");
        foreach ($GROUPED[$grouped_id] as $submessage) {
            $subpost_id = $submessage['id'] + 0;
            $wpdb->insert($post_table_name, array("created_at" => current_time('mysql', 1), "channel" => $task->channel, "post_id" => $subpost_id, "grouped_id" => $grouped_id));
            print time()-$NOW. "  Load submessage ".$subpost_id."\n";
            $post['post_content'] .= get_content($submessage, $MadelineProto);
        }
    }
    else {
        print (" ------- NO GROUPED  -----\n");
